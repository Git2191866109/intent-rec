...
def basic_seq2seq_core(w2v_dim, indices_dim, ques_token_len, ans_token_len):
...
#     loss = 'categorical_crossentropy'
    ...
    encoder_decoder.add(
        Masking(mask_value=0.0, input_shape=(ques_token_len, w2v_dim)))
...
#     token_len = max(ques_token_len, ans_token_len)
    vocab_dim = len(vocab)
    generator = basic_seq2seq_core(w2v_dim=w2v_model.vector_size, indices_dim=vocab_dim,
                          ques_token_len=ques_token_len, ans_token_len=ques_token_len)

    for _iter in range(0, nbIter):
        print('\n' + '-' * 50 + '\nIteration: {0}'.format(_iter))

        progress_bar = Progbar(target=len(corpus_tuple)
                               )  # set the progress bar
        for p in range(0, len(corpus_tuple), batch_size):
            progress_bar.update(p)
            # corpus_tuple_part is from index p to p + batch_size in all
            # corpus_tuple
...
def storageGenerator(generator, frame_path):
    ...
    generator.save_weights(record_path, overwrite=True)
...
def recompileGenerator(generator):
    ...
    generator.compile(loss='categorical_crossentropy', optimizer=optimizer, metrics=[
